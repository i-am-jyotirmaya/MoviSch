...
export const DetailsSlice = createSlice({
  ...
  reducers: {
    ...
    resetState: (state) => {
      ...
      state.Metascore = undefined;
      state.imdbRating = undefined;
      state.imdbVotes = undefined;
      state.Type = undefined;
      state.DVD = undefined;
      state.BoxOffice = undefined;
...
export const { fetchDetails, resetState } = DetailsSlice.actions;

export const fetchDetailsAsync = (imdbId) => async (dispatch) => {
  const omdb = new Omdb();
  omdb.fetchDetails(imdbId).then((res) => dispatch(fetchDetails(res)));
};
